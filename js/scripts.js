...
Post.prototype.pushReply = function(reply) {
  reply.id = this.assignId();
...
Post.prototype.createPost = function() {
  ...
  $("#results").append("<div class='container well style-post' id='first-post-" + topicsObject.currentId + "'><h2>" + this.header + "</h2>" + this.post + "<br>" + this.name + "<br>" + theCurrentTime.toDateString() + createReplyLink(topicsObject.currentId,0) + "</div>")
}
...
function displayReply(id, replyname, replymessage, e){
  var theCurrentTime = new Date();
  var postId = $(e.target).attr("data-postid");
  var replyId = $(e.target).attr("data-id");
  // $("#reply-msg-00").append(createReplyLink(postId, replyId))
  // $("#first-post-0").append(createReplyLink(postId, replyId))
  return "Name: "+ replyname + "<br>Time: " + theCurrentTime.toDateString() + "<br>Message: " + replymessage + "<br>" + createReplyLink(postId, replyId + 1);
}
...
$(document).ready(function(){
  ...
  $("#results").on("click", ".btn-reply-submit", function(e) {
    var replyId = $(e.target).attr("data-id");
    var postId = $(e.target).attr("data-postid");
    console.log("you are here!! reply ID: " + replyId);
    ...
    $("#reply-msg-" + postId + replyId).html(displayReply(replyId, replyName, replyMessage, e));
    var nextId = parseInt(replyId) + 1;
    console.log("you are there!! next ID: " + nextId);
    if (replyId === 1) {
